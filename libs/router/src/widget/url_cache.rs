...
impl RouterWidget {
    pub(super) fn parse_url_cached(&mut self, input: &str) -> RouterUrl {
        ...
        {
            let (k, v) = self.url_parse_cache.remove(pos);
            self.url_parse_cache.insert(0, (k, v.clone()));
            return v;
        }

        let parsed = RouterUrl::parse(input);
        self.url_parse_cache
            .insert(0, (input.to_string(), parsed.clone()));
        const MAX: usize = 8;
        if self.url_parse_cache.len() > MAX {
            self.url_parse_cache.truncate(MAX);
        }
        parsed
    }
}
